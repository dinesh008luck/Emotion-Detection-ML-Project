...
# Setup Logger
def setup_logger(log_file='feature_engineering.log', logger_name='feature_engineering_logger'):
...
def save_data(train_df, test_df, output_path):
    try:
        ...
        train_df.to_csv(os.path.join(output_path, "feature_train.csv"), index=False)
        test_df.to_csv(os.path.join(output_path, "feature_test.csv"), index=False)
